...
所以对于任何S(i'>=i, j'<=j) >= S(i,j)，由于j'-i' <= j-i，必然要有min(ai',aj')>=
min(ai,aj)才行。同样可以采用头尾双指针向中间移动：
...
public:
    int maxArea(vector<int>& height) {
        ...
        int vol = min(height[left], height[right]) * (n - 1);
        int curVol = 0;
        while (left < right) {
            curVol = min(height[left], height[right]) * (right - left);
            vol = max(vol, curVol);
