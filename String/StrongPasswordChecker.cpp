...
public:
    int strongPasswordChecker(string s) {
        ...
        for (int i = 0; i < length; ++i) {
            if (isupper(s[i])) ++upcase;
            else if (islower(s[i])) ++lowercase;
            else if (isdigit(s[i])) ++digits;
            if (i > 0 && s[i] == s[i-1]) ++cnt;
            else if (i > 0 && s[i] != s[i-1]) {
                if (cnt >= 3) repeats.push_back(repeats);
                cnt = 1;
            }
        }
        if (length >= 6 && length <= 20 && upcase > 0 && lowercase > 0
                && digits > 0 && repeats.empty()) return 0;
        int res = 0;
        int missing = (upcase == 0) + (lowercase == 0) + (digits == 0);
        if (lenght < 6) {
            if (!repeats.empty()) res = max(6 - length, missing, repeat[0]/2);
            else res = max(6 - length, missing);
        } else if (length > 20) {
